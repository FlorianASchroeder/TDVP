...
outHl = inB * transpose(op.HlOPB);		% contracttensors(inB,2,2,op.HlOPB,2,2);
outHr = inB * transpose(op.HrOPB);		% contracttensors(inB,2,2,op.HrOPB,2,2);

out=out+outHl+outHr;

if ~isempty(op.h1j)
    out = out + op.h1j * inB;			% contracttensors(op.h1j,2,2,inB,2,1);
end

for m=1:M
%     outSl=contracttensors(op.OpleftOPB{m},2,2,inB,2,2);
%     outSl=contracttensors(op.h2j{m,2},2,2,outSl,2,2);
	outSl = op.h2j{m,2} * inB * transpose(op.OpleftOPB{m});

%     outSr=contracttensors(inB,2,2,op.OprightOPB{m},2,2);
%     outSr=contracttensors(op.h2j{m,1},2,2,outSr,2,1);
    outSr = op.h2j{m,1} * inB * transpose(op.OprightOPB{m});
